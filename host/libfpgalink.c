...
/* Our wrappers for the read functions */

DLLEXPORT(FLStatus) customflReadChannel(
	struct FLContext *handle, uint8 chan, size_t count, uint8 *buffer,
	const char **error, uint32 timeout)
{
	...
	fStatus = customflReadChannelhelper(handle, chan, (uint32)count, buffer, error, timeout);
	...
	CHECK_STATUS(fStatus, fStatus, cleanup, "flReadChannel() 5");
	CHECK_STATUS(
		actualLength != requestLength,
		FL_EARLY_TERM, cleanup, "flReadChannel() 6");
cleanup:
	return retVal;
}


DLLEXPORT(FLStatus) customflReadChannelhelper(
	struct FLContext *handle, uint8 chan, uint32 count, uint8 *buffer, const char **error, uint32 timeout)
{
	FLStatus retVal = FL_SUCCESS, fStatus;
	uint8 command[3];
	USBStatus uStatus;
	...
	uStatus = customUsbBulkRead(
		...
		// printf("USTATUSCHECK %d\n",uStatus);
		// fflush(stdout);
	// Check for errors
	if(uStatus == USB_TIMEOUT){
		// printf("Timeout 1\n");
		// fflush(stdout);
		retVal = FL_TIMEOUT;
		return retVal;
